...
trait PlayJsonSupport {
  implicit def playJsonUnmarshaller[T: Reads] =
    PlayJsonUnmarshaller.delegate[String, T](`application/json`)(string ⇒
      try {
  ...
  //
  private final object PlayJsonUnmarshaller {
    def delegate[A, B](unmarshalFrom: ContentTypeRange*)(f: A ⇒ Deserialized[B])(implicit ma: Unmarshaller[A]): Unmarshaller[B] =
